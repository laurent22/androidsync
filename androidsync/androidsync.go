...
func (this *AndroSync) GetFolderItems(folderPath string) ([]FolderItem, error) {
	...
	for _, line := range lines {
		...
		if isDirectory { folderItem.path += this.PathSeparator }
...
func (this *AndroSync) Synchronize(androidPath string, localPath string) error {
	...
	for _, folderItem := range folderItems {
		if folderItem.isDirectory {
			err = this.Synchronize(folderItem.path, localPath + folderItem.name + this.PathSeparator)
			if err != nil {
				return err
			}
			continue
		...
		this.Logger.Print(folderItem.path + "...")
		cmd := exec.Command(this.AdbPath, "pull", folderItem.path, localPath + folderItem.name)
		var stdout bytes.Buffer
		cmd.Stdout = &stdout
		var stderr bytes.Buffer
		cmd.Stderr = &stderr
...
func (this *AndroSync) PatternMatchesFile(pattern string, filePath string) (bool, error) {	
	...
	if pattern[0] == '*' {
		pattern = this.PathSeparator + pattern
	} else if string(pattern[0]) != this.PathSeparator {
		pattern = this.PathSeparator + "*" + this.PathSeparator + pattern
	}
